...
function createUserData(isCompanyMember = true) {
    ...
    return {
        ...
        lastLoginAt: faker_1.faker.date.between({
...
async function main() {
    ...
    const teams = [];
    const shuffledTeamNames = faker_1.faker.helpers.shuffle([...TEAM_NAMES]);
    ...
    for (let i = 0; i < ownerCount; i++) {
        const user = await prisma.user.create({
            data: {
                ...
                teamLinks: {
                    create: faker_1.faker.helpers.arrayElements(teams.map(team => ({ teamId: team.id })), faker_1.faker.number.int({ min: 1, max: 5 })),
                },
            },
        });
        users.push(user);
    ...
    for (let i = 0; i < MEMBERS_COUNT - ownerCount; i++) {
        const user = await prisma.user.create({
            data: {
                ...
                teamLinks: {
                    create: faker_1.faker.helpers.arrayElements(teams.map(team => ({ teamId: team.id })), faker_1.faker.number.int({ min: 0, max: 3 })),
                },
            },
        });
        users.push(user);
    ...
    for (let i = 0; i < viewerCount; i++) {
        ...
        const user = await prisma.user.create({
            data: {
                ...
                teamLinks: {
                    create: faker_1.faker.helpers.arrayElements(teams.map(team => ({ teamId: team.id })), faker_1.faker.number.int({ min: 0, max: 1 })),
                },
            },
        });
        users.push(user);
    ...
    for (let i = 0; i < guestCount; i++) {
        const user = await prisma.user.create({
            data: {
                ...
                teamLinks: {
                    create: faker_1.faker.helpers.maybe(() => [{ teamId: faker_1.faker.helpers.arrayElement(teams).id }], { probability: 0.7 }),
                },
            },
        });
        users.push(user);
    ...
    for (let i = 0; i < ADDITIONAL_USERS_WITHOUT_MEMBERSHIP; i++) {
        const user = await prisma.user.create({
            data: {
                ...
                teamLinks: {
                    create: faker_1.faker.helpers.maybe(() => [{ teamId: faker_1.faker.helpers.arrayElement(teams).id }], { probability: 0.3 }),
                },
            },
        });
        users.push(user);
